...
usage() {
  echo "Usage: add-site-app-v2.sh -d domain.com -php 8.3 -ssl le -app wordpress|general [-http3]"
  exit 1
}
...
# ====================
# DOMAIN → DEFAULT USERNAME (strip dots)
# ====================
...
FPM_SOCK="/run/php/php-fpm-$USER.sock"
...
mkdir -p "$USER_HOME/tmp/php_sessions"
...
# ====================
# CONFIGURE PHP-FPM (per-user, not per-domain)
# ====================
calc_max_children() {
  local ram_mb
...
if [[ ! -f "$FPM_CONF" ]]; then
  log "Creating PHP-FPM pool for user $USER..."
  mycurl "$POOL_URL" > "$FPM_CONF"

  sed -i "s/\[custom\.com\]/[$USER]/g" "$FPM_CONF"
  sed -i "s/custom\.com/$USER/g" "$FPM_CONF"
  sed -i "s/stack-custom/$USER/g" "$FPM_CONF"
  sed -i "s|php_admin_value\[open_basedir\].*|php_admin_value[open_basedir] = /home/$USER:/tmp:/usr/share/php:/usr/lib/php:/var/cache/nginx/cache|" "$FPM_CONF"
  ...
  log "[PHP-FPM] Pool created for $USER (max_children=$maxc)"
else
  log "[PHP-FPM] Pool already exists for user $USER, skipping creation"
fi
...
sed -i "s|/home/$USER/logs/nginx/access.log|/home/$USER/logs/$DOMAIN/nginx/access.log|g" "$NGINX_CONF"
sed -i "s|/home/$USER/logs/nginx/error.log|/home/$USER/logs/$DOMAIN/nginx/error.log|g" "$NGINX_CONF"
sed -i "s|unix:/run/php/php-fpm-$DOMAIN.sock|unix:$FPM_SOCK|g" "$NGINX_CONF"
...
log ">> [LOGROTATE] Creating/updating LOGROTATE for $USER..."
...
echo "PHP Pool:       $FPM_CONF (shared per-user)"
